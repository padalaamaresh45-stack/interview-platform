...
def test_reassign_interviewer_allowed_only_when_not_started(client, db_session):
    admin = _admin_client(client, db_session)
    position = _make_position(db_session, admin)
    _make_question(db_session, position)
    interviewer_a = _make_user(db_session, email="a@example.com", role=UserRole.interviewer)
    interviewer_b = _make_user(db_session, email="b@example.com", role=UserRole.interviewer)

    not_started = _make_candidate(db_session, admin, position, interviewer_a)
    ok_resp = client.patch(
        f"/api/admin/candidates/{not_started.id}", json={"interviewer_id": interviewer_b.id}
    )
    assert ok_resp.status_code == 200
    assert ok_resp.json()["interviewer_id"] == interviewer_b.id

    completed = _make_candidate(db_session, admin, position, interviewer_a, status=CandidateStatus.completed)
    blocked_resp = client.patch(
        f"/api/admin/candidates/{completed.id}", json={"interviewer_id": interviewer_b.id}
    )
    assert blocked_resp.status_code == 400
    db_session.refresh(completed)
    assert completed.interviewer_id == interviewer_a.id
...
def test_deactivated_interviewer_excluded_from_picker_but_existing_assignment_unchanged(client, db_session):
    ...
    assert detail_resp.json()["interviewer_id"] == interviewer.id
