...
def _setup_two_questions(db_session):
    ...
    return admin, position, q1, q2, interviewer, candidate


def test_full_valid_submit_completes_and_leaves_queue(client, db_session):
    admin, position, q1, q2, interviewer, candidate = _setup_two_questions(db_session)
    _login_as(client, db_session, interviewer)

    resp = client.post(
        f"/api/interviewer/candidates/{candidate.id}/scores",
        json={"scores": [{"question_id": q1.id, "score": 4, "comment": "Good"}, {"question_id": q2.id, "score": 5}]},
    )
    assert resp.status_code == 200
    assert resp.json()["status"] == "completed"

    rows = db_session.query(InterviewScore).filter(InterviewScore.candidate_id == candidate.id).all()
    assert len(rows) == 2
...
def test_missing_question_returns_400_and_writes_nothing(client, db_session):
    ...
    assert db_session.query(InterviewScore).filter(InterviewScore.candidate_id == candidate.id).count() == 0
    db_session.refresh(candidate)
...
def test_resubmit_against_completed_candidate_returns_409_no_duplicates(client, db_session):
    admin, position, q1, q2, interviewer, candidate = _setup_two_questions(db_session)
    _login_as(client, db_session, interviewer)

    payload = {"scores": [{"question_id": q1.id, "score": 4}, {"question_id": q2.id, "score": 5}]}
    first = client.post(f"/api/interviewer/candidates/{candidate.id}/scores", json=payload)
    assert first.status_code == 200

    second = client.post(f"/api/interviewer/candidates/{candidate.id}/scores", json=payload)
...
def test_interviewer_a_cannot_open_or_submit_against_interviewer_b_candidate(client, db_session):
    ...
    submit_resp = client.post(
        f"/api/interviewer/candidates/{candidate_b.id}/scores",
        json={"scores": [{"question_id": q1.id, "score": 3}]},
    )
    assert submit_resp.status_code in (403, 404)
    assert db_session.query(InterviewScore).filter(InterviewScore.candidate_id == candidate_b.id).count() == 0
...
def test_concurrent_double_submit_yields_exactly_one_completed_and_one_409(client, db_session):
    ...
    def submit():
        resp = client.post(f"/api/interviewer/candidates/{candidate.id}/scores", json=payload)
